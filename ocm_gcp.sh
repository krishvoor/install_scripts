...
_update_maxUnavailable(){
    oc patch mcp worker --type='merge' --patch='{ "spec": { "maxUnavailable": "10%"}}'
}

_get_cluster_status(){
    echo "$(ocm describe cluster ${1} --json | jq -r [.state][])"
}

_wait_for_nodes_ready(){
    _download_kubeconfig "$(_get_cluster_id $1)" $HOME/kubeconfig
...
_wait_for_cluster_deleted(){
    ITERATIONS=0
    echo "INFO: Waiting 180 iterations until cluster is deleted"
    while [ ${ITERATIONS} -le 180 ]; do
        CLUSTER_STATUS="$(_get_cluster_status $1)"
        if [ -z "${CLUSTER_STATUS}" ]; then
            echo "INFO: ${ITERATIONS}/180. Cluster $1 removed."
            return 0
        elif [ "${CLUSTER_STATUS}" == "uninstalling" ]; then
            echo "INFO: ${ITERATIONS}/180. Cluster on ${CLUSTER_STATUS} status, waiting 5 seconds for next check"
            ITERATIONS=$((${ITERATIONS}+1))
            sleep 5
        else
            echo "ERROR: ${ITERATIONS}/180. Failed to delete cluster $1 after 180 loops, exiting..."
            return 1
        fi
    done
}
...
_wait_for_cluster_ready(){
    START_TIMER=$(date +%s)
    echo "INFO: Installation starts at $(date -r ${START_TIMER})"
    ...
    while [ ${ITERATIONS} -le 180 ]; do
        ...
        if [ "${CLUSTER_STATUS}" == "ready" ]; then
            ...
            echo "INFO: ${ITERATIONS}/180. Cluster on ${CLUSTER_STATUS} status, waiting 60 seconds for next check"
            ITERATIONS=$((${ITERATIONS}+1))
            sleep 60
        else
            # Sleep 1 to try to capture as much as possible states before installing
            sleep 1
        fi
        PREVIOUS_STATUS=${CLUSTER_STATUS}
    done
    echo "ERROR: Cluster $1 not installed after 180 iterations, dumping installation logs..."
    return 1
}
...
setup(){
    ...
    export ROSA_TOKEN=$(cat ${json_file} | jq -r .rosa_token)
    ...
    export GCP_MANAGED_SERVICES_TOKEN="/Users/krvoora/Downloads/managed-cloud-pltaforms-b635444413ed.json"
    ocm login --url=https://api.stage.openshift.com --token="${ROSA_TOKEN}"
    ocm whoami
    # gcloud config set account osd-ccs-admin@managed-cloud-pltaforms.iam.gserviceaccount.com
    # gcloud auth activate-service-account osd-ccs-admin@managed-cloud-pltaforms.iam.gserviceaccount.com --key-file ./serviceAccount.json
...
install(){
    ...
    ocm create cluster --ccs --provider gcp --region ${GCP_REGION} --service-account-file ${GCP_MANAGED_SERVICES_TOKEN} --channel-group ${MANAGED_CHANNEL_GROUP} --version ${OCM_VERSION} --multi-az --compute-nodes ${COMPUTE_WORKERS_NUMBER} --network-type ${NETWORK_TYPE} ${CLUSTER_NAME}
...
postinstall(){
    ...
    _download_kubeconfig "$(_get_cluster_id ${CLUSTER_NAME})" $HOME/kubeconfig
    unset KUBECONFIG
    kubectl delete secret ${KUBECONFIG_NAME} || true
    kubectl create secret generic ${KUBECONFIG_NAME} --from-file=config=$HOME/kubeconfig
